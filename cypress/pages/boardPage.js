...
class boardPage {
  ...
  createBoard() {
    ...
    this.chooseOwnerSelection.select('Sennder')
	  expect(cy.url().should('contain', 'boards/create')) // The string can also be put into a global variable...
    this.createBoardButton.click()
    expect(cy.url().should('contain', 'https://sprintboards.io/boards/')) // The string can also be put into a global variable...
